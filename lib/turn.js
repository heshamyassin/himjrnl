...
turnMethods = {
  ...
  center: function(page) {
    ...
    var data = this.data(),
      ...
      view = this.turn('view', page || data.tpage || data.page);

      if (deviceWidth <= responsiveViewTreshold) {
        if (!view[0]) {
          $('.magazine').css({left: ((deviceWidth - 1.5*size.width)/2)});
          $('.next-button').css({right: "0%"});
        } else if (!(data.page%2)) {
          $('.magazine').css({left: ((deviceWidth - 0.5*size.width)/2)});
          $('.previous-button').css({left: "0%"});
          $('.next-button').css({right: "50%"});
        } else if (data.page%2) {
          $('.magazine').css({left: ((deviceWidth - 1.5*size.width)/2)});
          $('.previous-button').css({left: "50%"});
          $('.next-button').css({right: "0%"});
        } else if (data.page>data.totalPages || !view[1]) {
          $('.magazine').css({left: ((deviceWidth - 1.5*size.width)/2)});
          $('.next-button').css({right: "0%"});
        }
      } else {
        ...
        if (!data.noCenter) {
          ...
          $(this).css({left: desktopLeft + ((deviceWidth - size.width)/2)});
        }
      }
    return this;
  },
  ...
  _view: function(page) {
    ...
    var data = this.data(), pageView;
    
    page = page || data.page;

    if (data.display=='double') {
      pageView = (page%2) ? [page-1, page] : [page, page+1];
      return pageView;
    } else
      return [page];
  ...
  view: function(page) {
    ...
    var data = this.data(),
      pageView = turnMethods._view.call(this, page);

    if (data.display=='double')
      return [(pageView[0]>0) ? pageView[0] : 0,
        (pageView[1]<=data.totalPages) ? pageView[1] : 0];
    else
      return [(pageView[0]>0 && pageView[0]<=data.totalPages) ? pageView[0] : 0];
  },
  ...
  next: function() {
    ...
    responsiveViewTreshold = 768;
    if (deviceWidth <= responsiveViewTreshold)
      return this.turn('page', $('.magazine').turn('page') + 1);
    else
